...
- [X] WebUI: три вкладки по задачам вместо одного набора настроек
  ...
  - Вкладки: **Распознавание целиком** (ручка OpenAI или нативная + формат, слова, VAD, `stream=true`), **Живая диктовка** (`WS /stt/stream`, настраивать нечего), **Распознавание эмоций** (`POST /stt/emotion`, текст не распознаётся).
