...
- [X] Фундамент для публикации образа + CI/CD на GitHub Actions
  ...
  - ⚠️ Release-воркфлоу ещё не запускался. Секреты Docker Hub добавлены, но у моего токена нет прав на `workflow_dispatch` — запуск руками: Actions → Release → Run workflow (сначала с `dry_run: true`).
