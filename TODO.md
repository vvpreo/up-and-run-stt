...
- [X] WebUI: три вкладки по задачам вместо одного набора настроек
  ...
  - Селектор языка переименован в «Язык в ответе»: проверено, что на распознавание он **не влияет вовсе** — `ru` и `en` дают побайтово одинаковый текст, модель русскоязычная. Параметр только проставляет поле `language` в ответе. Заодно он теперь отправляется и в OpenAI-ручку — раньше терялся, уходил только в нативную.
