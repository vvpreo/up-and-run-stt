...
- [X] WebUI: три вкладки по задачам вместо одного набора настроек
  - Наверху осталось только то, что не привязано к задаче: модель распознавания и язык. На вкладке эмоций они гасятся — эмоции считает отдельная модель, проверено (параметр `model` на `/stt/emotion` результат не меняет).
