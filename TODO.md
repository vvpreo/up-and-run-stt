...
- [X] WebUI: три вкладки по задачам вместо одного набора настроек
  - Модель и язык переехали внутрь вкладок, где применяются: на вкладке эмоций их нет вовсе (там своя модель, которой ни то, ни другое не передаётся). Блок один и переставляется между вкладками через `appendChild`, поэтому состояние селекторов не теряется и разметка не дублируется.
  - Селектор языка **убран** из консоли: по контракту OpenAI `language` опционален, а на распознавание он не влияет вовсе — `ru` и `en` дают побайтово одинаковый текст (модель русскоязычная), параметр только проставляет поле `language` в ответе. Контрол, который не может изменить результат, — ложное обещание. **Сервер параметр принимать не перестал** (проверено на обеих ручках), консоль его просто не задаёт и получает `DEFAULT_LANGUAGE`.
  - Вкладки в порядке: **Живая диктовка** (`WS /stt/stream`, настраивать нечего, открыта по умолчанию), **Распознавание целиком** (ручка OpenAI или нативная + формат, слова, VAD, `stream=true`), **Распознавание эмоций** (`POST /stt/emotion`, текст не распознаётся).
  - VAD-тумблер остался только на первой вкладке: в живой диктовке его отключить нельзя, сегментатор без него не найдёт границы фраз. Раньше тумблер там врал.
  - Вкладка эмоций скрывается целиком, если `ENABLE_EMOTIONS=false` на сервере.
  - Убрано накопление PCM под эмоции в живом режиме (добавлял в прошлой итерации) — при раскладке по вкладкам эмоции имеют свой ввод, и держать в странице ~115 МБ на час записи незачем.
  - Блок ввода общий под вкладками; на живой вкладке половина с файлом гасится.
