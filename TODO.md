...
- [ ] Публикация на Docker Hub: секреты добавлены, осталось запустить Actions → Release (сначала `dry_run: true`, затем боевой). Опционально: зеркало образа в GHCR, trivy-скан, вернуть 55 МБ зеркалированием лёгкого ffmpeg
